...
import { Button, Badge, Select, Input } from '@it-master-ai/ui';
...
const grades = [
  ...
  { level: 7, title: 'Programming Basics', modules: 10, lessons: 55, hours: 40, difficulty: 'Beginner', color: 'from-blue-500 to-blue-600', icon: Code, topics: ['Variables & Data Types', 'Control Flow (If/Else)', 'Loops (For/While)', 'Functions', 'Lists & Dictionaries', 'Basic Algorithms', 'Debugging', 'Project: Calculator', 'Project: Guessing Game', 'Project: Quiz App'] },
  { grade: 8, title: 'Data & Logic', modules: 10, lessons: 60, hours: 45, difficulty: 'Intermediate', color: 'from-purple-500 to-purple-600', icon: Database, topics: ['Data Structures', 'Search & Sort Algorithms', 'Binary & Hexadecimal', 'Logic Gates', 'Boolean Algebra', 'Introduction to Python', 'File I/O', 'Error Handling', 'Project: Text Analyzer', 'Project: Data Visualizer'] },
  { grade: 9, title: 'Web Development', modules: 12, lessons: 70, hours: 55, difficulty: 'Intermediate', color: 'from-pink-500 to-pink-600', icon: Globe, topics: ['HTML5 Semantic Elements', 'CSS3 & Flexbox/Grid', 'Responsive Design', 'JavaScript Basics', 'DOM Manipulation', 'Events & Forms', 'ES6+ Features', 'Fetch API', 'Project: Portfolio Site', 'Project: Weather App', 'Project: Todo App', 'Deploying to Netlify'] },
  { grade: 10, title: 'Advanced Programming', modules: 12, lessons: 75, hours: 60, difficulty: 'Advanced', color: 'from-orange-500 to-orange-600', icon: Layers, topics: ['Object-Oriented Programming', 'Databases & SQL', 'REST APIs', 'Authentication', 'Git & GitHub', 'Testing (Unit/E2E)', 'CI/CD Basics', 'Docker Intro', 'Project: Blog API', 'Project: E-commerce Backend', 'Project: Real-time Chat', 'Project: Full Stack App'] },
  { grade: 11, title: 'Specialization Tracks', modules: 14, lessons: 85, hours: 70, difficulty: 'Advanced', color: 'from-red-500 to-red-600', icon: Lock, topics: ['Cybersecurity Fundamentals', 'Network Security', 'Ethical Hacking Basics', 'AI/ML Introduction', 'Neural Networks', 'Cloud Computing (AWS/Azure)', 'DevOps Basics', 'Capstone Project', 'Portfolio Building', 'Industry Certifications Prep'] },
];
...
export function CoursesPage() {
  ...
  const filteredGrades = grades.filter(grade => {
    const matchesGrade = !selectedGrade || selectedGrade === String(grade.grade);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {grades.map(grade => (
              <button
                key={grade.grade}
                onClick={() => setSelectedGrade(selectedGrade === String(grade.grade) ? null : String(grade.grade))}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  selectedGrade === String(grade.grade)
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGrades.map((grade, index) => (
              <motion.div
                ...
                className="group bg-white dark:bg-dark-bg rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-xl hover:border-primary-300 dark:hover:border-primary-700 transition-all duration-300"
              >
                <div className="p-6">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    ...
                    <Badge variant="outline" className="text-xs">{grade.difficulty}</Badge>
                  </div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Grade {grade.grade}</h3>
                  <p className="text-primary-600 dark:text-primary-400 font-medium mb-4">{grade.title}</p>
                  ...
                  <Link to={`/courses/${grade.grade}`} className="block w-full text-center">
...
// Need to import the missing icons
import { Code, Database, Globe, Layers, Lock } from 'lucide-react';